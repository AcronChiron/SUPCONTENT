"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || (function () {
    var ownKeys = function(o) {
        ownKeys = Object.getOwnPropertyNames || function (o) {
            var ar = [];
            for (var k in o) if (Object.prototype.hasOwnProperty.call(o, k)) ar[ar.length] = k;
            return ar;
        };
        return ownKeys(o);
    };
    return function (mod) {
        if (mod && mod.__esModule) return mod;
        var result = {};
        if (mod != null) for (var k = ownKeys(mod), i = 0; i < k.length; i++) if (k[i] !== "default") __createBinding(result, mod, k[i]);
        __setModuleDefault(result, mod);
        return result;
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
exports.unbanUser = exports.banUser = exports.featureReview = exports.resolveReport = exports.getReports = void 0;
const zod_1 = require("zod");
const asyncHandler_1 = require("../utils/asyncHandler");
const pagination_1 = require("../utils/pagination");
const adminService = __importStar(require("../services/admin"));
const resolveSchema = zod_1.z.object({ status: zod_1.z.enum(['RESOLVED', 'DISMISSED']) });
const featureSchema = zod_1.z.object({ featured: zod_1.z.boolean() });
exports.getReports = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const pagination = (0, pagination_1.parsePagination)(req.query);
    const { data, total } = await adminService.getReports(pagination, req.query.status);
    res.json((0, pagination_1.paginate)(data, total, pagination));
});
exports.resolveReport = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { status } = resolveSchema.parse(req.body);
    const report = await adminService.resolveReport(req.params.reportId, status);
    res.json(report);
});
exports.featureReview = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { featured } = featureSchema.parse(req.body);
    const review = await adminService.featureReview(req.params.reviewId, featured);
    res.json(review);
});
exports.banUser = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    await adminService.banUser(req.params.username);
    res.json({ message: 'User banned' });
});
exports.unbanUser = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    await adminService.unbanUser(req.params.username);
    res.json({ message: 'User unbanned' });
});
//# sourceMappingURL=admin.js.map